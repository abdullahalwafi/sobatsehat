...
@section('konten')
    ...
    @foreach ($kegiatan as $k)
        <form method="POST" action="{{ url('/kegiatan/update/' . $k->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group row">
                ...
                <div class="col-8">
                    <input id="nama_kegiatan" name="nama_kegiatan" placeholder="Masukkan Nama Kegiatan" type="text"
                        class="form-control" value="{{ $k->nama_kegiatan }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-8">
                    <select id="kategori_id" name="kategori_id" class="custom-select" aria-describedby="kategori_idHelpBlock">
                        @foreach ($kategori as $i)
                            <option value="{{ $i->id }}" {{ $k->kategori_id == $i->id ? 'selected' : '' }}>
            ...
            <div class="form-group row">
                ...
                <div class="col-8">
                    <input id="lokasi" name="lokasi" placeholder="Masukkan Lokasi" type="text" class="form-control " value="{{ $k->lokasi }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="waktu" class="col-4 col-form-label">Waktu Pelaksanaan</label>
                <div class="col-8">
                    <input id="waktu" name="waktu" type="date" class="form-control" value="{{ $k->waktu }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="inputer" class="col-4 col-form-label">Penginput</label>
                <div class="col-8">
                    <input id="inputer" name="inputer" type="text" class="form-control" value="{{ $k->inputer }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="deskripsi" class="col-4 col-form-label">Deskripsi</label>
                <div class="col-8">
                    <textarea id="deskripsi" name="deskripsi" cols="40" rows="5" class="form-control" value="{{ $k->deskripsi }}"></textarea>
                </div>
            </div>
            <div class="form-group row">
