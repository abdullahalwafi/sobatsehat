            ...
            <div class="card-body">
                ...
                <form action="{{ url('dashboard/lokasi/update', $lokasi->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        ...
                        <input id="nama" name="nama" value="{{ $lokasi->nama }}" type="text" class="form-control col-8" required>
                    </div>
                    <div class="form-group row">
                        <label for="alamat" class="col-4 col-form-label">Alamat</label>

                        <input id="alamat" name="alamat" value="{{ $lokasi->alamat }}" type="text" class="form-control col-8" required>
                    </div>

                    <div class="form-group row">
                        <label for="province" class="col-4">Provinsi</label>
                        <select name="province" id="province" class="form-control col-8" required>
                            <option value="{{ $lokasi->provinces_id }}">{{ $lokasi->province->name }}</option>
                        </select>
                    </div>
                    <div class="form-group row">
                        <label for="city" class="col-4">Kota/Kab</label>
                        <select name="city" id="city" class="form-control col-8" required>
                            <option value="{{ $lokasi->cities_id }}">{{ $lokasi->cities->name }}</option>
                        </select>
                    </div>
                    <div class="form-group row">
                        <label for="district" class="col-4">Kecamatan</label>
                        <select name="district" id="district" class="form-control col-8" required>
                            <option value="{{ $lokasi->districts_id }}">{{ $lokasi->districts->name }}</option>
                        </select>
                    </div>
                    <div class="form-group row">
                        <label for="village" class="col-4">Kelurahan</label>
                        <select name="village" id="village" class="form-control col-8" required>
                            <option value="{{ $lokasi->villages_id }}">{{ $lokasi->villages->name }}</option>
                        </select>
                    </div>
                    <div class="form-group row">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const populateProvinceSelect = (data) => {
                provinceSelect.innerHTML = '<option value="{{ $lokasi->provinces_id }}">{{ $lokasi->province->name }}</option>';
